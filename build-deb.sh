...
clean_build() {
    ...
    # Try to clean normally first
    if debian/rules clean >/dev/null 2>&1; then
        print_success "Build artifacts cleaned with debian/rules clean"
    else
        print_warning "Normal clean failed, trying with sudo..."
        ...
        print_success "Build artifacts cleaned with sudo"
    fi
    ...
    rm -f ../${PACKAGE_NAME}_*.dsc 2>/dev/null || true
    rm -f ../${PACKAGE_NAME}_*.changes 2>/dev/null || true
    rm -f ../${PACKAGE_NAME}_*.buildinfo 2>/dev/null || true
    rm -f ../${PACKAGE_NAME}_*.tar.* 2>/dev/null || true

    # Remove dist directory
    rm -rf dist

    # Remove upstream tarball
    rm -f ../${PACKAGE_NAME}_*.orig.tar.gz 2>/dev/null || true
...
# Function to create upstream tarball
create_upstream_tarball() {
    print_status "Creating upstream tarball..."
    
    # Get version from changelog
    local version=$(dpkg-parsechangelog -S Version | cut -d'-' -f1)
    local package_name=$(dpkg-parsechangelog -S Source)
    
    # Create tarball name
    local tarball_name="${package_name}_${version}.orig.tar.gz"
    local tarball_path="../${tarball_name}"
    
    # Check if tarball already exists
    if [ -f "$tarball_path" ]; then
        print_status "Upstream tarball already exists: $tarball_path"
        return 0
    fi
    
    # Create tarball from current directory, excluding debian and build artifacts
    tar --exclude='.git' \
        --exclude='.gitignore' \
        --exclude='.gitmodules' \
        --exclude='.venv' \
    ...
    if [ $? -eq 0 ]; then
        print_success "Upstream tarball created: $tarball_path"
    else
        print_error "Failed to create upstream tarball"
        return 1
    fi
}
...
build_binary_for_arch() {
    ...
    # Create upstream tarball first (required for source format 3.0 native)
    create_upstream_tarball
